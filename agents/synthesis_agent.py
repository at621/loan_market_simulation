...
class SynthesisAgent:
    ...
    async def synthesize_final_lessons(self,
                                     megarun_results: List[Dict],
                                     megarun_lessons: List[str],
                                     hypothesis_results: List[Dict]) -> Tuple[List[SynthesisInsight], str]:
    ...
    def _identify_cross_megarun_patterns(self, megarun_results: List[Dict]) -> Dict:
        ...
        patterns['equity_stability_correlation'] = self._analyze_equity_stability_pattern(configs, outcomes)
        patterns['strategy_concentration_pattern'] = self._analyze_strategy_concentration_pattern(configs, outcomes)
        patterns['image_competition_pattern'] = self._analyze_image_competition_pattern(configs, outcomes)
        patterns['hypothesis_validation_summary'] = self._summarize_hypothesis_validations(megarun_results)
        
        return patterns
        
    def _analyze_equity_stability_pattern(self, configs: List[Dict], outcomes: List[Dict]) -> Dict:
        ...
        return {
            ...
            'pattern': 'higher_equity_more_stable' if len(equity_stability_pairs) > 1 and 
                      equity_stability_pairs[-1][1] > equity_stability_pairs[0][1] and
                      equity_stability_pairs[-1][0] > equity_stability_pairs[0][0] else 'mixed'
        }
        
    def _analyze_strategy_concentration_pattern(self, configs: List[Dict], outcomes: List[Dict]) -> Dict:
        ...
        concentrations = [outcome.get('final_market_concentration_hhi', 0) for outcome in outcomes]
        
        return {
            'strategy_concentration_pairs': list(zip(grow_pcts, concentrations)),
            'pattern': 'more_growth_more_concentration' if len(grow_pcts) > 1 and
                      any(g > 0.6 for g in grow_pcts) and 
                      max(concentrations) > min(concentrations) * 1.5 else 'stable'
        }
        
    def _analyze_image_competition_pattern(self, configs: List[Dict], outcomes: List[Dict]) -> Dict:
    ...
    async def _generate_synthesis_insights(self,
                                         megarun_results: List[Dict],
                                         megarun_lessons: List[str],
                                         hypothesis_results: List[Dict],
                                         patterns: Dict) -> List[SynthesisInsight]:
...
Prioritize insights with strong evidence from multiple megaruns."""
        ...
        messages = [
            SystemMessage(content="You are a senior market design expert synthesizing insights from extensive empirical research."),
    ...
    def _format_megarun_results(self, results: List[Dict]) -> str:
        ...
        for i, result in enumerate(results, 1):
            ...
            lines.append(f"  Config: {config.get('grow_strategy_pct', 0):.0%} Growth, "
                        f"${config.get('avg_equity', 0):.0f}M avg equity, "
                        f"{config.get('avg_image_score', 0):.2f} avg image")
            lines.append(f"  Outcome: {outcome.get('survival_rate', 0):.0%} survival, "
    ...
    def _format_hypothesis_results(self, results: List[Dict]) -> str:
        ...
        for i, result in enumerate(results, 1):
            if result:
                ...
                lines.append(f"  Result: {result.get('result', '')} ({result.get('confidence', 0):.0%})")
    ...
    def _parse_synthesis_response(self, response: str, megarun_results: List[Dict]) -> List[SynthesisInsight]:
        ...
        for match in matches:
            ...
            megarun_numbers = []
            megarun_matches = re.findall(r'megarun\s+(\d+)', evidence_text, re.IGNORECASE)
    ...
    def _generate_final_report(self,
        ...
        for insight_type, type_insights in by_type.items():
            ...
            for insight in type_insights:
                ...
                megaruns_str = ", ".join(map(str, insight.megaruns_involved)) if insight.megaruns_involved else "All"
                lines.append(f"- **[{confidence_pct}% confidence, Megaruns {megaruns_str}]** {insight.insight_text}")
        ...
        for i, result in enumerate(megarun_results, 1):
            ...
            lines.append(f"\n### Megarun {i}")
            lines.append(f"**Configuration**: {config.get('grow_strategy_pct', 0):.0%} Growth strategy, "
                        f"${config.get('avg_equity', 0):,.0f}M average equity")
            lines.append(f"**Results**: {outcome.get('survival_rate', 0):.0%} survival rate, "
                        f"{outcome.get('total_bankruptcies', 0)} bankruptcies, "
                        f"{outcome.get('final_market_concentration_hhi', 0):.0f} HHI")
            
            if hypothesis:
                lines.append(f"**Hypothesis**: {hypothesis.get('hypothesis', '')}")
                lines.append(f"**Result**: {hypothesis.get('result', '')} ({hypothesis.get('confidence', 0):.0%} confidence)")
                
        # Add recommendations
        lines.append(f"\n## Recommendations for Market Designers")
        recommendations = [insight for insight in insights if insight.insight_type == 'recommendation']
        if recommendations:
            for rec in recommendations:
                lines.append(f"- {rec.insight_text}")
        else:
            lines.append("- Based on the analysis, focus on configurations that balance growth and stability")
            lines.append("- Consider equity requirements as a key stability factor")
            lines.append("- Monitor market concentration to prevent monopolistic outcomes")
