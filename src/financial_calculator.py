...
class FinancialCalculator:
    ...
    def amortize_portfolio(
        self, portfolio: pd.DataFrame, current_round: int, amortization_years: int
    ) -> pd.DataFrame:
        """
        ...
        rounds_since_origination = (
            current_round - portfolio.loc[active_mask, "round_added"]
        )
    ...
    def calculate_bank_pnl(
        ...
        market_history: Optional[List] = None,
    ) -> BankFinancials:
        """
        ...
        bank_portfolio = portfolio[
            (portfolio["bank_id"] == bank_id) & (portfolio["is_active"] == True)
        ]
        ...
        if not bank_portfolio.empty:
            ...
            old_book_income = (
                ...
                * bank_portfolio["interest_rate_bps"]
                / 10000
            ).sum()

        # 2. Calculate new book metrics
        new_loan_ids = [
            c_id for c_id, b_id in consumer_allocations.items() if b_id == bank_id
        ]
        ...
        if market_history and len(market_history) > 0:
            ...
            if original_volume > 0:
                ...
                if volume_ratio < 0.3:
                    is_bankrupt = True
                    logger.warning(
                        f"Bank {bank_id} bankrupt due to volume decline: "
    ...
    def calculate_portfolio_metrics(self, portfolio: pd.DataFrame) -> Dict:
        ...
        metrics = {
            ...
            "total_principal_outstanding": active_portfolio[
                "principal_outstanding"
            ].sum(),
            "average_rate": (
                active_portfolio["interest_rate_bps"].mean()
                if len(active_portfolio) > 0
                else 0
            ),
            "weighted_average_rate": (
                (
                    (
                        active_portfolio["principal_outstanding"]
                        * active_portfolio["interest_rate_bps"]
                    ).sum()
                    / active_portfolio["principal_outstanding"].sum()
                )
                if active_portfolio["principal_outstanding"].sum() > 0
                else 0
            ),
        }
        ...
        for bank_id in portfolio["bank_id"].unique():
            ...
            bank_portfolios[bank_id] = {
                ...
                "average_rate": (
                    bank_active["interest_rate_bps"].mean()
                    if len(bank_active) > 0
                    else 0
                ),
            }
